...
Backport of PR #351 to the 2.14 line: import paths adjusted
vesmaro->mnemos (pre-rebrand package), content otherwise verbatim.

``mnemos serve`` must, when ``mesh.enabled`` is true:

* start the :class:`mnemos.mesh_server.MeshServer` on the configured
...
from mnemos.cli.main import serve
...
def _serve_env() -> Any:
    ...
    """
    keys = ("MNEMOS_API__HOST", "MNEMOS_API__PORT")
...
class TestServeMeshDisabled:
    def test_mesh_disabled_starts_uvicorn_only(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("mnemos.mesh_server.MeshServer") as mesh_cls,
            patch("mnemos.api.main.get_manager") as get_mgr,
        ):
...
class TestServeMeshEnabled:
    def test_starts_mesh_server_and_drains_on_return(self, tmp_path: Path) -> None:
        ...
        mesh_logger = logging.getLogger("mnemos.mesh_server")
