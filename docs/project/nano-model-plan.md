...
## 8. Выбор базовых моделей (ресерч 2026-09-01, URL-верифицирован)

**Учитель для дистилляции (этап NM-1b):**
| Кандидат | Лицензия | Параметры | RU/качество | Вердикт |
|---|---|---|---|---|
| **Qwen3-Embedding-0.6B** | Apache-2.0 | 0.6B | MTEB-Multilingual 64.33 — топ открытого класса <1B; MRL 32–1024 (матрёшка) | ✅ **выбран** |
| multilingual-e5-large | MIT | 560M | де-факто RU-стандарт | контрольная нога качества |
| BGE-M3 | MIT | 560M | dense+sparse+ColBERT | запасной |
| jina-embeddings-v3 / Cohere | CC-BY-NC | — | — | ❌ некоммерческие |

**Рефайнер (этап NM-3):** Qwen3-0.6B (Apache-2.0, RU в мультитюн-сете, int4 ~0.4–0.5 ГБ) — основной; Qwen2.5-0.5B-Instruct — запасной. SmolLM2 — EN-only, отсечён.

**По модели «ornith» (вопрос владельца):** веб-ресерч (2026-09-01) указывает на `ornith-ai/Ornith-1.5` (9B/35B/397B, MIT, линейж поверх Qwen3.5+Gemma4) — это **чат/резонинг LLM**, не эмбеддер и не нано-класс; в наш класс не подходит ни учителем, ни рефайнером. Независимая верификация карточки ограничена (HF отдаёт 401 без сессии) — при первом касании проверить live. Возможно, модель встретилась в трендах HF.

**Ниша подтверждена пустой:** готового мультиязычного трансформерного эмбеддера <100M не существует (MiniLM-m 118M — выше бюджета; potion — статика с потерями; bge-micro — EN-only; rubert-tiny2 — RU-центричный со слабым EN). Наш студент 45–60M закрывает реальный пробел рынка.

**Рецепт KD** — стандарт де-факто MSE-on-cosine (прецедент bge-micro-v2, MIT); опционально матрёшечные головы от MRL-учителя Qwen3 (головы 256/384/512).

**Менеджер обучения** — `training/train.py`: подкоманды `status / prepare / train (resume) / export / eval / snapshot / doctor`; состояние в `state.json`, стоп-флаг (мягкая остановка на границе эпохи), снапшоты чекпоинтов с метриками, doctor-проверка окружения (torch/IPEX/диск). Идемпотентен: повторный запуск пропускает готовые стадии, resume продолжает с последней завершённой эпохи.
