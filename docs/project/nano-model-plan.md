...
**Рефайнер (этап NM-3):** **Qwen3.5-0.8B** (Apache-2.0, 2026-02, 256K контекст, 201 язык, свежее Qwen3-0.6B на 8 месяцев; оговорка: это VLM — при текст-only vision-башня игнорируется; RU-поддержка подтверждается смоук-тестом суммаризации при первом касании) — основной; Qwen3-0.6B int4 — запасной; Qwen2.5-0.5B — резерв. SmolLM2/SmolLM3-класс — EN-only (карта модели; live-проверка при первом касании), отсечён. LFM2.5-350M (июль 2026, RU в 15 языках) — свеж, но base-модель под свой SFT и лицензионно менее удобен.

**Примечание по «Qwen 3.8» (владелец, 2026-09-01):** линейка Qwen3.8 вышла (август 2026: 27B VLM / 2.4T-A95B MoE / Flash-Next ~180B, Apache-2.0), но в ней НЕТ ни эмбеддеров, ни моделей <27B — для обеих наших ролей не подходит. Qwen3.5-Embedding не выпускался; учитель-эмбеддер Qwen3-Embedding-0.6B остаётся самым свежим открытым мультиязычным (проверено по MTEB-релизам за август 2026).
