...
контекста (ADR-0018) в main. Впереди — измерительная часть (стенды/базлайны
ADR-0020) и уборка (Фаза D). ИТОГО программа Фазы 2 ≈ 55 %.**
...
| ADR-0020    | 20 % (рамка ✅ · стенды 0/4)          | #169 БФ-1                  |
...
flowchart TD
    ...
    BF1["#169 · БФ-1 · эпик бенчмарков<br/>benchmarks/ в корне: миграция корпуса<br/>из tests/golden, S1-расширение,<br/>JSON-базлайны + генератор BASELINE.md"]
...
- **#169 (БФ-1) — вход всей измерительной части.** Каталог `benchmarks/` в
  корне проекта (директива владельца 2026-08-30, поправка ADR-0020 §4):
  миграция корпуса из `tests/golden`, расширение S1, JSON-базлайны и
  генератор `BASELINE.md`. S1 остаётся в локальном merge-гейте.
- **#170 ∥ #171 ∥ #185** — параллельные лейны после БФ-1; все трое гейтят
  БФ-2 (зависимости версий/стендов).
