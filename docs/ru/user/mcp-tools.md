...
1. **существование** — запись найдена по `(project, hash)`; строгая валидация требует область `project` (поиск без области выкупил бы маркер по первой сохранённой копии любого проекта);
