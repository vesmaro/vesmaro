...
**Строгая валидация маркера A2.** Запрос имеет *форму маркера*, если несёт любое из полей `original_chars` / `agent` / `session`. В строгом режиме запрос формы маркера обязан пройти: (1) **существование** по `(project, hash)` — область `project` в строгом режиме обязательна; (2) **целостность** — `N` маркера равен длине сохранённого оригинала в символах; (3) **происхождение** — реестр эмитентов строки кэша (записан при сжатии) совпадает с парой `(agent, session)` вызывающего (`null`-сессия соответствует только `null`-сессии эмитента). Непройденная проверка возвращает:
