...
- **Импорт** — `mnemos sync import` читает compact-payload
  ...
  под форму `CompactRecord` — `content=summary`, `title`, `tags`) и
  мержит идемпотентно по `id` записи (`fed:<source_agent>:<uuid>`
  prefix — существующие записи пропускаются, не перезаписываются). При
...
key points, теги и provenance — не сырой контент — поэтому радиус
поражения при утечке sync-файла ограничен уже промодерированными
summary.
