...
Когда апгрейд привозит новые встроенные веса эмбеддера (например, замена
round-3 для `mnema-embed-v1`, `weights_sha256 3b752e06…`), существующие
векторы созданы старой геометрией. Вручную ничего делать не нужно:
...
embedding-пространства — семантическое ранжирование может слегка
деградировать; полнотекстовый поиск не затронут.
