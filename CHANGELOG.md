...
MAJOR — engine-level breaking shift (versioning policy v2, directive 5a5ac447): the bundled nano-embedder becomes the DEFAULT embedder and chromadb is removed from the runtime. Covers the 10 PRs merged to `main` after `v3.2.0`: #210, #212, #214, #216, #217, #218, #219, #220, #221, #222.

> **Breaking for chromadb-provider users** — migration: switch to the bundled default embedder (`provider='nano'`, zero-config zero-key, bundled `nano-embed-v1` int8 ONNX, 384-dim); `onnxruntime`, `tokenizers` and `huggingface_hub` are now direct dependencies.
>
> **PyPI reconciliation** — the 3.2.0 PyPI upload carried the nano-model without a changelog entry, and the uploaded wheel does not match the tagged tree — reconciled here. v4.0.0 is the first pipeline-governed release (release branch → marker sync → merge commit → annotated tag → GitHub Release). The artifact rename `nano-embed-v1` → `mnema-embed-v1` (feat/nm1d-mnema-rename-docs) is in flight and lands after v4.0.0.

### Changed

- **NM-1c — NanoProvider default with bundled `nano-embed-v1`; chromadb removed from runtime (ADR-0021, BREAKING)** (#221) — the bundled nano-embedder (`src/mnemos/models/nano-embed-v1/`: model.onnx 29.6 MB int8 ONNX + tokenizer + manifest, 384-dim) is now the DEFAULT embedder: zero-config, zero-API-key vector memory out of the box. chromadb is gone from the runtime (onnxruntime/tokenizers/huggingface_hub became direct dependencies). The S1m production contour was honestly re-recorded (ADR-0020 trigger: embedder change) with the new model fingerprint.
...
- **NM-1a — nano-model training skeleton (ADR-0021, epic #197)** (#212) — dataset prep, knowledge-distillation (KD) training loop, int8 ONNX export, eval rig and pilot pipeline under `training/`.
- **NM-1a.5 — training manager CLI + base-model research** (#214) — teacher-selection research (Qwen3-Embedding teacher; ornith verdict) and CLI orchestration for the training pipeline.
- **mnemos-train binary + `training` extra** (#216) — fresh-model check with the Qwen3.5-0.8B refiner reference.
...
- **NM-1b pilot round** (#218) — projector saved in checkpoints (co-adaptation fix), export includes the projection head, graph dedup for torch-exporter duplicate tensor names, static-1 int8 calibration, eval batch-1 adapter.
- **BF-2 review round** (#217) — F1 source-pin isolation test (mutation kills the tautology), F2 ruff-clean new files plus `benchmarks/` in the lint gate.
- **BF-4 review round F1–F4** (#222) — ruff clean (EN DASH + newlines), NOISE symmetric p50+p95, record refused at NOISE, smoke cannot green F1.
