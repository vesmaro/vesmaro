...
(:func:`vesmaro.pipeline.publish.publish_memory`); the secrets patterns
themselves are NOT duplicated here — the secret class **delegates** to
:func:`vesmaro.secrets_detector.detect_secrets` (the existing single
...
* ``secret`` — high-confidence secret patterns only: the discrete
  structural patterns of :mod:`vesmaro.secrets_detector` (aws-key,
...
Design constraints (mirroring :mod:`vesmaro.secrets_detector`)
-------------------------------------------------------------
...
#: :func:`vesmaro.secrets_detector.detect_secrets`).
...
#: declared in ``vesmaro.secrets_detector._PATTERNS``; the
...
#: renames in :mod:`vesmaro.secrets_detector`.
