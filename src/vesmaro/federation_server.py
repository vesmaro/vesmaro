...
FastAPI app (see :mod:`vesmaro.api.federation`) and reuses the Phase 1
prerequisite modules:

* :mod:`vesmaro.trigger_codes` — the five trigger codes (contract §9).
* :mod:`vesmaro.federation_access_log` — append-only audit log for
  anti-correlation tracking (contract §10, КП-5).
* :mod:`vesmaro.config` — :class:`FederationConfig` /
  :class:`PeerConfig` (per-peer ACL, ADR-0016).
* :mod:`vesmaro.moderation` — :func:`moderate` (no verdict cache, §0.п.6).
* :mod:`vesmaro.compact` — :func:`build_compact_record` for the sanitized
  response shape (contract §2.3).
* :mod:`vesmaro.manager` — :class:`MemoryManager.search` for the local
  search leg.
...
from vesmaro.moderation import moderate
...
class PullResponse(BaseModel):
    ...
    Fields:
        trigger_code: One of the five :class:`TriggerCode` values. A
            dispatches on this (see :mod:`vesmaro.federation_client`).
...
class PullResult(BaseModel):
    """A-side result of a federation pull (see :mod:`vesmaro.federation_client`).
...
def _memory_type_for_filter(tags: list[str]) -> str:
    ...
    Mirrors :func:`vesmaro.compact.derive_record_type` but returns the raw
