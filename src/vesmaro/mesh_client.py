...
marshal :class:`~vesmaro.compact.CompactRecord` to/from the protobuf
``CompactRecord``.
...
Architectural invariants (ArchCom 2026-07-17 federation contract):
    ...
    * **Criterion 2 — moderation stays in Python.** The mesh never runs
      ...
      compact-payload build time (see :mod:`vesmaro.compact`).
...
def _compact_to_proto(record: CompactRecord) -> Any:
    ...
    :mod:`vesmaro.compact`). No transformation is applied: the record is
    ...
    :mod:`vesmaro._mesh_gen`); callers pass it straight back into the gRPC
