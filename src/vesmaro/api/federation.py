...
see :func:`vesmaro.federation_server.verify_mtls_fingerprint`).
"""
...
from vesmaro.federation_access_log import DEFAULT_LOG_PATH, FederationAccessLog
from vesmaro.federation_server import PullRequest, PullResponse, handle_pull
...
def get_manager(request: Request) -> MemoryManager:
    ...
    Mirrors :func:`vesmaro.api.main.get_manager` but is route-local so
...
async def federation_pull(
    ...
    See :func:`vesmaro.federation_server.handle_pull` for the flow. The
